...
Deno.serve(async (req) => {
  ...
  try {
    const body = (await req.json()) as EndEventBody;
    ...
    const { data, error } = await supabase
      .from("rooms")
      .update({ status: "archive", ceremony_ended_at: new Date().toISOString() })
      .eq("id", body.roomId)
      .eq("host_user_id", body.hostUserId)
      .select("id, status, ceremony_ended_at")
      .single();
    ...
    return new Response(
      JSON.stringify({
        roomId: data.id,
        status: data.status,
        ceremonyEndedAt: data.ceremony_ended_at,
      }),
      {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
